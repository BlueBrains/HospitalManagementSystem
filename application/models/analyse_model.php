...
class analyse_model extends  CI_Model
{
    public function get_information($id)
	 ...
	 {
	 	 $q=$this->db->query("SELECT COUNT(id)as not_number FROM analyse_request WHERE recived_by_admin=0 and sended=1");
         if($q->num_rows()>0)
         {
              foreach ($q->result() as $raw ) {
                $data[]=$raw;
	 ...
	 public function get_notification()
     {
         $q=$this->db->query("SELECT analyse_request.id as id,state,patients.name as Pname,doctors.name as Dname,analyse_name as Nname FROM analyse_request
                             INNER JOIN patients on patients.id=analyse_request.patient_id
                              ...
                              WHERE recived_by_admin=0 and sended=1
         ");
         if($q->num_rows()>0)
         {
              $x=$this->db->query("UPDATE analyse_request SET recived_by_admin=1 WHERE recived_by_admin=0");
              foreach ($q->result() as $raw ) {
                $data[]=$raw;
            }
            return $data;
