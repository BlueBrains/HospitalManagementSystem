...
class supervisor_model extends CI_Model {
	...
	function all_doctonu()
	{
		$id=$this->ion_auth->user()->row()->profile_id;
		$sql=$this->db->query("SELECT * FROM doctonur_calls  where sender_id = '".$id."' and checked=0");
			foreach ($sql->result() as $raw ) {
                $data[]=$raw;
            }
		if ($sql->num_rows >= 1)
